...
import { AutomodPane } from "~/components/dashboard/automod/AutomodPane";
...
export default function HubWorkspace({ params }: Route.ComponentProps) {
  ...
  const { data: rules = [] } = useQuery(orpc.moderation.getAutomodRules.queryOptions({ input: { hubId }, enabled: view === "safety" }));
  ...
  return <>
    ...
    {view === "safety" && <><section className="dashboard-section"><div className="dashboard-grid"><div className="dashboard-panel dashboard-panel--wide"><div className="dashboard-panel__body"><h3>Protections</h3><p>{rules.length} AutoMod {rules.length === 1 ? "rule" : "rules"} configured. Held messages, reports, and appeals appear in the safety inbox.</p><div className="dashboard-actions" style={{ marginTop: 16 }}><a className="dashboard-button dashboard-button--primary" href={`/dashboard/inbox?hubId=${hubId}`}>Open safety inbox</a></div></div></div><div className="dashboard-panel dashboard-panel--narrow"><div className="dashboard-panel__body"><h3>Image filtering</h3><p>{(hub.spec.settings & 32) === 32 ? "Explicit images are blocked before delivery." : "Standard Hub image policy applies."}</p></div></div></div></section><section className="dashboard-section"><div className="dashboard-section__header"><h2>AutoMod rules</h2></div><AutomodPane hubId={hubId} canManageRules={hub.metadata.permissions.MANAGE_RULES} /></section></>}
